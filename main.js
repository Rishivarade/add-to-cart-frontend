...
let PitchData = []
function showData() {
    let result = fetch('https://add-to-cart-backend-m5fc.onrender.com/pitches').then(e => e.json()).then(e => {
        PitchData = e
        e.map(e => {
...
pitchCreateBtn.addEventListener('click', () => {
    ...
    let fetchPost = fetch('https://add-to-cart-backend-m5fc.onrender.com/pitches', {
        method: 'POST',
...
function deletePitch(id) {
    let delPitch = fetch(`https://add-to-cart-backend-m5fc.onrender.com/pitches/${id}`, {
        method: 'DELETE'
    })
...
function EditPitch(id) {
    fetch(`https://add-to-cart-backend-m5fc.onrender.com/pitches/${id}`).then(e => e.json()).then(e => {
...
updatePitchBtn.addEventListener('click', (e) => {
    ...
    fetch(`https://add-to-cart-backend-m5fc.onrender.com/pitches/${updatePitch.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
updatePricePitchPriceButton.addEventListener('click', () => {
    ...
    fetch(`https://add-to-cart-backend-m5fc.onrender.com/pitches/${priceId.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
